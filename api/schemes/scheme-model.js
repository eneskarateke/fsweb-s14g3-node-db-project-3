...
async function findById(scheme_id) {
  ...
  */
  ...
  const unwantedSchema = await db("schemes as sc")
    .leftJoin("steps as st", "sc.scheme_id", "st.scheme_id")
    .select("sc.scheme_name", "st.*")
    .where("sc.scheme_id", scheme_id)
    .orderBy("st.step_number", "asc");

  if (unwantedSchema.length == 0) {
    return null;
  }

  const result = {
    scheme_id: Number(scheme_id),
    scheme_name: unwantedSchema[0].scheme_name,
    steps: [],
  };
  if (unwantedSchema[0].step_id === null) {
    return result;
  }

  for (let i = 0; i < unwantedSchema.length; i++) {
    const item = unwantedSchema[i];
    let stepModel = {
      step_id: item.step_id,
      step_number: item.step_number,
      instructions: item.instructions,
    };
    result.steps.push(stepModel);
  }
  return result;
}
